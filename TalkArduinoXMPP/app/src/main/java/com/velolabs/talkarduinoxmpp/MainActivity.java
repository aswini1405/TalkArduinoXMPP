package com.velolabs.talkarduinoxmpp;
...
public class chatStarr extends Activity {
    ...
    String statString;
    String firstTxt = "Chat initialized...  Stay tuned for messages from Android.";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        statString = "Welcome to Chat.  Click \"O N\" to get started.";
    ...
    protected void onStart() {
        super.onStart();
        registerReceiver(aR, new IntentFilter("RESPONSE"));

        // are the setCollection commands necessary?
        Intent setCollection = new Intent("SET_COLLECTION");
    ...
    public class buttonClick implements Button.OnClickListener {
        public void onClick(View bClick) {
            if ((bClick == onOff) && (isOn == false)) {
                isOn = true;
                onOff.setText("O F F");
                statString = "Chat starting...";
                makeConnection();
                displayStatus.setText(statString);
            } else if ((bClick == onOff) && (isOn == true)) {
                isOn = false;
                onOff.setText("O N");
                statString = "Chat turning off...";
                displayStatus.setText(statString);
                theCon.disconnect();
            }
        }
